...
def contact(request):
    return render(request, "villa_booking_app/contact.html")
...
def villa_detail(request, villa_id):
    ...
    booked_dates = Booking.objects.filter(villa=villa).values_list(
        ...
        if form.is_valid():
            ...
            if Booking.objects.filter(
                (
                    ...
                    | (
                        Q(check_in_date__lte=booking.check_in_date)
                        & Q(check_out_date__gte=booking.check_out_date)
                    )
                ),
                villa=villa,
            ).exists():
                return render(
                    request,
                    "villa_booking_app/booking_error.html",
                    {
